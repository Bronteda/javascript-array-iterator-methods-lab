//Inventors array ( Array of Objects)
const inventors = [
  ...
  //Comments Array (Array Of Objects)
  const comments = [
...
//Adding the filtered Inventors into a new array called veryOldInventors
...
firstLast = people.map((person)=>{
   const personsName = person.split(",")
   return personsName[1] + " " + personsName[0];
});
...
// Complete the exercise in the space below:
//I got the dynamic year from above in exercise 6
isEveryone19OrOlder = devs.every((dev)=>{
    return year - dev.year >= 19;
})
...
//This one was tricky had to do a bit of googling to understand this method
...
travelMethodCounts = travelMethods.reduce((acc, method) => {
  ...
  // acc['car'] = (acc['car'] || 0) + 1;
  // acc['car'] doesn't exist yet → undefined
  // (undefined || 0) = 0 → 0 + 1 = 1
  // So now: acc = { car: 1 }
